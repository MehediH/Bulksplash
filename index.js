...
const bulksplash = async (args) => {
      ...
      .prompt([
        {
          ...
          name: 'saveCredits',
          message:
                        '🗂  Export the credits for the photos to a .json file?',
          default: true,
        },
      ])
      .then((answers) => {
  ...
  if (args.length != 0) {
    ...
    options.width =
            args.w && parseInt(args.w) > 0 ? parseInt(args.w) : null
    options.height =
            args.h && parseInt(args.h) > 0 ? parseInt(args.h) : null
    options.orientation =
  ...
  console.log(
    `\n🔰 Downloading ${options.amount}${
      options.featured ? ' featured' : ''
    }${options.search ? ' "' + options.search + '"' : ''} images from:`
  )
